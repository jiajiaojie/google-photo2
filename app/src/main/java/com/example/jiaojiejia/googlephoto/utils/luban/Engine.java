...
class Engine {
    ...
    private boolean isJpeg(File photo) {
        return photo.getAbsolutePath().toLowerCase().contains("jpeg") || photo.getAbsolutePath().toLowerCase().contains("jpg");
    }
    ...
    String autoRotate() throws IOException {
        if (srcExif == null
                || srcExif.getAttributeInt(ExifInterface.TAG_ORIENTATION, ExifInterface.ORIENTATION_NORMAL) == ExifInterface.ORIENTATION_NORMAL
                || srcExif.getAttributeInt(ExifInterface.TAG_ORIENTATION, ExifInterface.ORIENTATION_NORMAL) == ExifInterface.ORIENTATION_UNDEFINED) {
            return srcImg.getAbsolutePath();
        }
        if(tagImg.exists()) {
            return tagImg.getAbsolutePath();
        }
        BitmapFactory.Options options = new BitmapFactory.Options();
        options.inSampleSize = computeSize();   // TODO: 2017/12/28 旋转原图OOM
    ...
    String handRotate(int degree) throws IOException {
        BitmapFactory.Options options = new BitmapFactory.Options();
